...
const Tunggakan = ({ items, user, short, shortKelas, relasi }) => {
    ...
    const handleShortData = (shorting) => {
        setLoading(true);
        router.get(route('tunggakan'), { short: shorting }, {
            onSuccess: () => {
                setLoading(false);
            }
        });
    ...
    const handleShortKelas = (shortKelas) => {
        ...
        if (!shortKelas.toLowerCase().includes('semua')) {
            router.get(route('tunggakan'), { short_kelas: shortKelas }, {
                onSuccess: () => {
                    setLoading(false);
                }
            });
        } else {
            router.get(route('tunggakan'));
        }
    }
